...
typedef struct {
    ...
    char grade;
                 ...
                 // Making Function Calculate _grade
void* calculate_grade(void* arg) {
    ...
    // here we are printing the student information
    printf("Student ID: %d\n", student->student_id);
...
int main(void){ 
    //data passing to the threads through local array of struct i-e Students declared above
    Students students[3]= {
        {1, "Dawood Saif", 90, ' '},
        {2, "Abdul Rehman", 80, ' '},
        {3, "Laiba Saif", 60, ' '},  
    ...
    for(int i=0; i<3; i++ ){
        ...
        // Here we are passing 4 arguments to thread using built in function pthread_create
    }
